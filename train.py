...
# if __name__ == "__main__":
#     # Ensure it saves locally
#     mlflow.set_tracking_uri("file://" + os.path.abspath("mlruns"))
    
#     with mlflow.start_run() as run:
#         mlflow.log_metric("accuracy", 0.95)
        
#         # FIX: Changed f.read = f.write to just f.write
#         with open("model_info.txt", "w") as f:
#             f.write(run.info.run_id)
        
#         print(f" Success! Logged Accuracy: 0.95")

import mlflow
import os
import sys

# 1. Point to the local data
tracking_uri = "file://" + os.path.abspath("mlruns")
mlflow.set_tracking_uri(tracking_uri)

# 2. Check if the ID file exists
...
try:
    # 3. Pull the accuracy from the local MLflow folder
